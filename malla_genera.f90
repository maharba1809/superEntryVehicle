...
!--------------------------------------------------------------
        if(regularx.eq.'si')then
           ...
           DO i=1,nx1
              x(i)=(1.)*(FLOAT(i-1)/float(nx1-1))**potx1
           ENDDO
           DO i=1,nx2
              x(i+nx1)=(1.)*(FLOAT(i)/float(nx2))**potx2+x(nx1)
           ENDDO
           DO i=1,nx3
              x(i+nx1+nx2)=(lx-x(nx1+nx2))*(FLOAT(i)/float(nx3))**potx3+x(nx2+nx1)
           ENDDO   

        open(59,file='mesh/funx.dat')
        DO i=1,nx
!           write(*,*)i,x(i)
           write(59,*)i,x(i)
        end do
...
!--------------------------------------------------------------
      ...
      open(54,file='mesh/ass_malla.dat')
        ...
        ENDDO
      close(54)
      Write(*,*)'Crea--> ass_malla.dat'
      end if

      open(11,file='mesh/jet.grid',form='unformatted')
      write(11)nv
      write(11)nx,ny,nz
      WRITE(11)xx
      WRITE(11)yy
