...
open(66,file='mesh/jet.grid',form='unformatted')
    read(66)in_Nvar
    read(66)nx,ny,nz
...
300  format(i3.3)
   write(*,*)'Reading field_'//serie//'.'//campo//''
   open(19,file='fields/field_'//serie//'.'//campo//'',form='unformatted')
       READ(19)in_Nvar
       READ(19)nx,ny,nz
       READ(19)u
       READ(19)v
   ...
   open(234,file='output/jet_'//serie//'_'//campo//'.dat')
     ...
     do k=nz/2,nz/2
        do j=1,ny
           do i=1,nx
            rho=p(i,j,k)/t(i,j,k)
            ma = ((u(i,j,k)**2 + v(i,j,k)**2 + w(i,j,k)**2)**0.5)/(t(i,j,k)**0.5) *1.4
