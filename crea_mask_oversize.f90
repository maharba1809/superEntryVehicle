...
program mascara
use vargrid
open(53,file='jet.grid',form='unformatted')
      read(53)nv
      read(53)nx,ny,nz
...
if((rcon1.le.tan(teta1)*(x(i,j,k)-xc1i)).and.(x(i,j,k).le.xc1f).and.x(i,j,k).ge.xe)mask(i,j,k)=0.0
 ...
 !cilindro
if((x(i,j,k).ge.xi).and.(x(i,j,k).le.xf).and.(rcon1.le.0.5))mask(i,j,k)=0.0
 !cono 2
...
open(55,file='mask.grid',form='unformatted')
    write(55)mask
close(55)
